# -*- coding: utf-8 -*-
"""
TESTS TO CHECK VARIOUS FUNCTIONALITIES OF CHITTER APPLICATION.
"""
...
BASE_URL2 = "http://13.40.180.161:8000"


def registerNewUser(username, password, firstname, lastname):
    import requests
    url = BASE_URL2 + "/authentication/register/"
    userObject = {
        'username': username,
        'password': password,
...
def requestNewToken(username, password):
    import requests
    url = BASE_URL2 + "/authentication/token/"
    requestObject = {
        'username': username,
        'password': password,
    }
...
def refreshToken(rToken):
    import requests
    url = BASE_URL2 + "/authentication/token/refresh/"
    requestObject = {
...
def revokeToken(token):
    import requests
    url = BASE_URL2 + "/authentication/token/revoke/"
    requestObject = {
        'token': token
    }
...
"""
TC 1. Olga, Nick, Mary and Nestor register and are ready to access the CHITTER API.
"""

print('\n\n\nTC 1. Olga, Nick, Mary and Nestor register and are ready to access the CHITTER API.\n\n\n')

# get token if the user is already registered
def try_register_or_get_token(username, password, first_name, last_name):
    user_data = registerNewUser(username, password, first_name, last_name)
    if user_data is None or "a user with that username already exists." in str(user_data.get("username", "")).lower():
        print(f"User '{username}' already exists. Requesting a new token...")
        return requestNewToken(username, password)
    return user_data

olga = try_register_or_get_token("olga", "olga123", "Olga", "Kiev")
nick = try_register_or_get_token("nick", "nick123", "Nick", "English")
mary = try_register_or_get_token("mary", "mary123", "Mary", "Johns")
nester = try_register_or_get_token("nester", "nester123", "Nester", "Smith")
...
message_URL = BASE_URL2 + "/v1/message/"
...
# message_URL = BASE_URL2 + "/v1/topic/"
...
# message_URL = BASE_URL2 + "/v1/message/"
# access_token = olga['access_token']
# nester_headers = {'Authorization': 'Bearer ' + str(access_token)}
# dataset = {
#
#     "topic": [
#         {
...
# }
#
# olga_request_to_post = requests.post(message_URL, headers=nester_headers, json=dataset)
...
# }
#
# nick_request_to_post = requests.post(message_URL, headers=nester_headers, json=dataset)
...
# }
#
# mary_request_to_post = requests.post(message_URL, headers=nester_headers, json=dataset)
...
tech_posts_URL = BASE_URL2 + "/v1/messagebytopic/T/"
access_token = nick['access_token']
nester_headers = {'Authorization': 'Bearer ' + str(access_token)}

nick_request = requests.get(tech_posts_URL, headers=nester_headers)
...
access_token = olga['access_token']
nester_headers = {'Authorization': 'Bearer ' + str(access_token)}
olga_request = requests.get(tech_posts_URL, headers=nester_headers)
...
access_token = nick['access_token']
feedback_URL = BASE_URL2 + "/v1/feedback/"
tech_posts_URL = BASE_URL2 + "/v1/messagebytopic/T/"
nester_headers = {'Authorization': 'Bearer ' + str(access_token)}

marys_post_identifier = 0
post_count = 0
posts_in_tech = requests.get(tech_posts_URL, headers=nester_headers)
...
if post_count != 1:
    ...
    # post request with nick's access token used above
    submit_request = requests.post(feedback_URL, headers=nester_headers, json=dataset)
...
"""
TC 10. Nick browses all the available posts in the Tech topic at this stage,
...
as in CHITTER a post owner cannot like their own message.
"""
print(
    '\n\n\nTC 11. Mary likes her post in the Tech topic. This call should be unsuccessful, as in CHITTER a post owner cannot like their own message.\n\n\n')
...
fashion(one after another adding at least 2 comments each)
"""
...
feedback_URL = BASE_URL2 + "/v1/feedback/"
tech_posts_URL = BASE_URL2 + "/v1/messagebytopic/T/"
nester_headers = {'Authorization': 'Bearer ' + str(nick_access_token)}
...
if post_count != 1:
    ...
    nester_headers = {'Authorization': 'Bearer ' + str(olga_access_token)}
...
nester_request_to_post = requests.post(message_URL, headers=nester_headers, json=dataset)
...
health_posts_URL = BASE_URL2 + "/v1/messagebytopic/H/"
access_token = mary['access_token']
nester_headers = {'Authorization': 'Bearer ' + str(access_token)}

marys_request = requests.get(health_posts_URL, headers=nester_headers)
...
feedback_URL = BASE_URL2 + "v1/feedback/"
health_posts_URL = BASE_URL2 + "v1/messagebytopic/H/"
nester_headers = {'Authorization': 'Bearer ' + str(mary_access_token)}

nestor_post_identifier = 0
post_count = 0
posts_in_health = requests.get(health_posts_URL, headers=nester_headers)
...
access_token = nester['access_token']
nester_headers = {'Authorization': 'Bearer ' + str(access_token)}
nester_request = requests.get(health_posts_URL, headers=nester_headers)
...
sports_posts_URL = BASE_URL2 + "/v1/expiredmessagebytopic/S/"
access_token = nick['access_token']
nester_headers = {'Authorization': 'Bearer ' + str(access_token)}

nick_request = requests.get(sports_posts_URL, headers=nester_headers)
...
tech_posts_URL = BASE_URL2 + "/v1/messagebytopicsorted/T/"
access_token = nester['access_token']
nester_headers = {'Authorization': 'Bearer ' + str(access_token)}

nester_request = requests.get(tech_posts_URL, headers=nester_headers)
